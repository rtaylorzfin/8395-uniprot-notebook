...
echo "\copy (select * from to_delete) to stdout with csv header"                                           | psql -h $DBHOST $DBNAME > to_delete.csv
echo "\copy (select * from feature_marker_relationship) to stdout with csv header"                         | psql -h $DBHOST $DBNAME > feature_marker_relationship.csv
echo "\copy (select * from external_note where extnote_note ilike '%ENSDARG%') to stdout with csv header"  | psql -h $DBHOST $DBNAME > extnote_note.csv
...
csv-to-sqlite -o zfin-db-slice.db -f refseq2ncbi.csv
